...
def preprocess(img):
    # Cropt images are
    img = img[35:195]
    # Donwsample by factor of 2
    img = img[::2,::2,0]
    # Erase background (background type 1)
    img[img == 144] = 0
    # Erase background type 2
    img[img == 109] = 0
...
class Model(object):
    ...
    def policy_forward(self, x):
        ...
        # Return propability of taking action 2 and hidden state
        return p, h
...
class RolloutWorker(object):
    ...
    def comptute_gradient(self, model):
        # Comptute a simulation episode
...
for i in range(1, 1+iterations):
    ...
    gradient_ids = [
        actor.comptute_gradient.remote(model_id) for actor in actors
    ]
